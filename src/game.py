...
class Game:
    ...
    def announce_winner(self):
        ...
        for player in self.players:
            if player.score > highest_player_score and player.score<22:
                highest_player_score = player.score
                highest_scoring_player = player

        #Non-21-dealer-win-case                
        if self.dealer.score <21 and self.dealer.score >= highest_player_score:
            return f"Dealer wins with {self.dealer.score} points."
        
        #Non-21-dealer-lose-case
        elif (self.dealer.score<21 and highest_player_score<21 and self.dealer.score<highest_player_score) or (self.dealer.score>21 and highest_player_score<21 and highest_player_score):
            winners = [highest_scoring_player.name]
            for player in self.players:
                if player.name != highest_scoring_player.name:
                    if player.score == highest_player_score:
        ...
        #21 points dealer cases 
        elif self.dealer.score == 21 and self.dealer.score > highest_player_score:
            if len(self.dealer.hand) ==2:
                return f"Dealer wins with a pontoon worth {self.dealer.score} points."
            elif len(self.dealer.hand) == 5:
                return f"Dealer wins with a five card trick worth {self.dealer.score} points."
            else: 
            ...
            elif len(self.dealer.hand) !=2 and len(highest_scoring_player.hand) ==2:
                
                pontoons = [highest_scoring_player]
                for player in self.players:
                    if player != highest_scoring_player:
                        if player.score == highest_player_score and len(player.hand)==2:
                            pontoons.append(player)
                if len(pontoons) ==1:
                    return f"{highest_scoring_player.name} wins with a pontoon worth {highest_player_score} points!"
                elif len(pontoons) ==2:
            ...
            elif len(self.dealer.hand) !=5 and len(highest_scoring_player.hand) == 5:
                five_card_tricks = [highest_scoring_player]
                for player in self.players:
                    if player != highest_scoring_player:
                        if player.score == highest_player_score and len(player.hand)==5:
        ...
        #21 points player cases
        elif (self.dealer.score <21 or self.dealer.score>21) and highest_player_score == 21:
            ...
            if len(winners) == 1:
                if len(highest_scoring_player.hand)==2:
                    return f"{highest_scoring_player.name} wins with a pontoon worth 21 points!"
                elif len(highest_scoring_player.hand) == 5:
                    return f"{highest_scoring_player.name} wins with a five card trick worth {highest_player_score} points!"
                else:
                    return f"{highest_scoring_player.name} wins with {highest_player_score} points!"
            else:
